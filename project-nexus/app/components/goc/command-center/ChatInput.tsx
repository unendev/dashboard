  ...
  return (
    <div className="p-4 border-t border-zinc-800 bg-[#0a0a0a]">
      <form onSubmit={onSubmit} className="flex gap-2 relative">
        ...
        <button
          type="submit"
          disabled={isLoading && aiModeEnabled}
          className="bg-cyan-900 hover:bg-cyan-800 text-cyan-100 px-6 py-2 rounded font-bold transition-colors border border-cyan-700 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isLoading && aiModeEnabled ? 'WAIT' : 'SEND'}
        </button>
      </form>
    </div>
  );
