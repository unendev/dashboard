...
export function TimerControls({
  ...
  onComplete,
}: TimerControlsProps & { onComplete?: (taskId: string) => void }) {
    ...
    >
      ...
      {onComplete && (
        <Button
          ...
          disabled={isDisabled}
          title="完成任务"
          className={`text-xs md:text-sm px-2 md:px-3 py-1.5 md:py-2 h-auto min-w-[32px] md:min-w-[36px] ${hasInstanceTag
      ...
      {task.isRunning ? (
        task.isPaused ? (
          ...
          <Button
            ...
            disabled={isDisabled}
            size="sm"
            className={`text-xs md:text-sm px-2 md:px-3 py-1.5 md:py-2 h-auto ${hasInstanceTag
